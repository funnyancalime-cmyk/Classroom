...
def export_arrangement_pdf(
    ...
    if not REPORTLAB_AVAILABLE:
        raise RuntimeError("PDF export vyžaduje knihovnu reportlab. Nainstaluj volitelné balíčky: pip install -r requirements-optional.txt")
...
class Database:
    ...
    def list_settings(self):
        rows = self.conn.execute("SELECT key, value FROM app_settings ORDER BY key").fetchall()
        return {r["key"]: r["value"] for r in rows}

    def replace_settings(self, settings: dict[str, str]):
        with self.conn:
...
class SeatingApp(tk.Tk):
    ...
    def open_settings_dialog(self):
        ...
        ttk.Button(controls, text="Export nastavení", command=self.export_settings_json).pack(side="left", padx=(0, 8))
        ttk.Button(controls, text="Import nastavení", command=self.import_settings_json).pack(side="left", padx=(0, 8))
    ...
    def export_settings_json(self):
        data = {
            ...
            "app_settings": self.db.list_settings(),
        }
        path = filedialog.asksaveasfilename(
            title="Export nastavení",
            defaultextension=".json",
            initialfile=f"seating_settings_{datetime.now().strftime('%Y%m%d_%H%M')}.json",
        ...
        try:
            ...
            messagebox.showerror("Chyba exportu", f"Nastavení se nepodařilo exportovat.\n\n{e}")
            return
        messagebox.showinfo("Export hotov", f"Nastavení bylo uloženo:\n{path}")
    ...
    def import_settings_json(self):
        ...
        try:
            ...
            if not isinstance(settings, dict):
                raise ValueError("Soubor neobsahuje platnou sekci 'app_settings'.")
            self.db.replace_settings(settings)
        except Exception as e:
            messagebox.showerror("Chyba importu", f"Nastavení se nepodařilo importovat.\n\n{e}")
            return
        messagebox.showinfo("Import hotov", "Nastavení bylo obnoveno.")
